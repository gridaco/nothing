...
| *(measured, not celled)* | An extreme pure percentage or percentage-only `calc()` takes a different width path: Chromium accepts it, but overflow during used-value multiplication produces join- and cap-dependent paint — under a discriminating transform, a butt-capped round or bevel join paints, while the default miter and round/square-cap variants do not. The named `svg-stroke-width-percentage-overflow` refusal guards that valid grammar class. It has no independent checklist row, so this corrects gridaco/nothing#80's evidence and reopens both `stroke-width` twins under the gridaco/nothing#81 split law; the fixed huge-length cell remains admitted. |
...
once. Strokes land in the same class and mostly below it: 97 of the 98
