...
| *(measured, not celled)* | `stroke-dashoffset` shifts the cycle for positive/negative unitless and px values plus normalized-diagonal percentages. `pathLength` calibrates dash values on path, rect, circle, and ellipse, and calibrates dash offset on path. Both remain named guarded refusals because this frame amendment is zero-phase and uncalibrated. Viewport/container/font-metric/root-font-metric unit classes, `var()` indirection, CSS escapes, and a poisoned em basis also remain registered refusals. Under the earned gridaco/nothing#77 and gridaco/nothing#80 standard-track and guarded-gap precedents, these do not hold `stroke-dasharray` open. The gridaco/nothing#81 split precedent remains valid, but its premise is absent here: the measured used-value behavior now has committed twin cells, so both dasharray rows tick. |
