...
| *(measured, not celled)* | `stroke-dashoffset` shifts the cycle for positive/negative unitless and px values plus normalized-diagonal percentages. `pathLength` calibrates dash values on path, rect, circle, and ellipse, and calibrates dash offset on path. Both remain named guarded refusals because this frame amendment is zero-phase and uncalibrated. Viewport/container/font-metric/root-font-metric unit classes, `var()` indirection, CSS escapes, and a poisoned em basis also remain registered refusals following the stroke-width/own-unit-row precedent. One additional in-grammar remainder is engine-refused: individually finite intervals whose f32 cycle sum overflows (`3.4e38 3.4e38`; the zero-painted round-cap variant proves the class cannot be normalized to solid). The cells land, but the attribute and CSS checklist twins stay unchecked as an explicit gridaco/nothing#81-style SPLIT until that representability rung closes. |
...
once. Strokes land in the same class and mostly below it: 94 of the 95
