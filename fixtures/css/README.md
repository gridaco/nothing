...
## [grida-markdown.css](../../crates/htmlcss/assets/css/grida-markdown.css)

Grida's own markdown stylesheet for the `htmlcss` render pipeline. Targets bare HTML elements produced by `pulldown-cmark` (GFM mode) under a `.markdown-body` wrapper. Uses element selectors only (no GitHub-specific classes), explicit px/rgb values (no CSS custom properties), and Grida's embedded font stack (Geist). Production ownership lives with the renderer crate at `crates/htmlcss/assets/css/grida-markdown.css`.
