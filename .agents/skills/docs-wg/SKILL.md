...
[`docs/AGENTS.md`](https://github.com/gridaco/grida/blob/main/docs/AGENTS.md) — read it once. For _reading_
the WG tree before you edit, use [`grounding`](https://github.com/gridaco/grida/blob/main/.agents/skills/grounding/SKILL.md).
...
- **Conversational logs / history / decision diaries.** "On Tuesday we
  ...
  [`docs/AGENTS.md`](https://github.com/gridaco/grida/blob/main/docs/AGENTS.md)), never in the live spec.
...
- WG docs in THIS repo live in the engine topic clusters: `docs/wg/canvas/`,
  `docs/wg/format/`, surveys in `docs/wg/research/`, and the engine
  `feat-*` clusters (feat-2d, feat-svg, feat-text-editing, feat-paragraph,
  feat-crdt, feat-history, feat-layout, feat-masks, …). Put the doc in the
  cluster that owns its topic; create a new `feat-<topic>/` cluster when
  none fits (consult [`naming`](../naming/SKILL.md) for the cluster name).
  Product/service-domain WG docs (platform, ai, desktop, editor/slides/
  svg-editor product clusters) are authored in the grida repo:
  https://github.com/gridaco/grida/tree/main/docs/wg.
- **Most clusters have an `index.md` hub.** When you add a doc, update the
  ...
  doc.
- **Frontmatter** (per [`docs/AGENTS.md`](https://github.com/gridaco/grida/blob/main/docs/AGENTS.md)):
  `title`, a `description`, `tags: [internal, wg, <topic>…]` drawn from
  the controlled vocabulary in [`docs/tags.yml`](https://github.com/gridaco/grida/blob/main/docs/tags.yml),
  and `format: md` for plain-Markdown pages (the MDX-safety opt-out).
  *Interim note: this repo's wg tree has no docs-site/frontmatter/tags
  pipeline yet — the linked grida conventions apply where sensible; do not
  invent a parallel vocabulary here.*
- **Links** follow the [`links`](../links/SKILL.md) skill. In this repo:
  engine paths are same-repo relative; grida-side references are absolute
  (`https://github.com/gridaco/grida/blob/main/<path>` or `https://grida.co/...`).
  The docs site does NOT publish this repo's wg tree — never author
  `https://grida.co/docs/wg/...` links for docs that live here.
...
[`docs`](https://github.com/gridaco/grida/blob/main/.agents/skills/docs/SKILL.md) (the family router — start there if unsure this
is even a WG doc), [`research`](../research/SKILL.md) (the `research/`
upstream-survey subtree), [`grounding`](https://github.com/gridaco/grida/blob/main/.agents/skills/grounding/SKILL.md) (read and
reconcile before editing), [`links`](../links/SKILL.md),
[`naming`](../naming/SKILL.md) (cluster and concept names).
Operational mechanics: [`docs/AGENTS.md`](https://github.com/gridaco/grida/blob/main/docs/AGENTS.md).
