...
4. Verify the round-trip with the Rust tests (the TS cross-boundary cycle is retired — see above).
...
1. Add a targeted Rust assertion — one assertion per bug, on the specific field that was wrong.
2. Do NOT use snapshot comparisons for codec bugs.
3. If the feature is a known limitation (e.g. scale/skew in transforms), gate the test with an explanatory comment.
