...
<p class="warning">Selected obligation: ${e(report.obligation)}. Engine-only: ${report.engine_ready ? "satisfied" : "NOT ready"}. Engine-only never certifies canonical reference reproduction.</p>
...
<p>Engine-only: ${e(r.engine_verdict.status)}. Reference identity: ${e(r.reference_problem ?? "matches declared profile")}</p><pre>${e(r.engine_verdict.reasons.join("\n"))}</pre>
...
<div class="images">${["baked", "chromium", "strict", "best", "stored", "resvg"]
      ...
      .join(
        ""
      )}</div><details><summary>Exact comparisons, diagnostics and both repeats</summary><pre>${e(JSON.stringify({ pairs: r.pairs, observations: r.observations, reference_identity: r.reference_identity }, null, 2))}</pre></details></section>`
  )
  .join("\n")}</html>`;
}
