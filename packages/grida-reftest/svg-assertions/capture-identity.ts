...
async function executableIdentity(path: string) {
  ...
  try {
    ...
    for await (const chunk of createReadStream(path, {
      fd: file.fd, autoClose: false, start: 0,
    })) {
      count += chunk.length;
    ...
    if (count !== before.size || before.size !== after.size ||
...
export async function captureIdentity(browser: Browser) {
  ...
  try {
    ...
    if (process.platform === "linux") executable = await readlink(`/proc/${pid}/exe`);
    else if (process.platform === "darwin") {
      executable = (await promisify(execFile)("ps", ["-p", String(pid), "-o", "comm="], {
    ...
    return {
      schema_version: 1,
      host: { platform: process.platform, arch: process.arch, release: release(), version: version() },
      browser: { product: browserVersion.product, revision: browserVersion.revision, ...binary },
      raster: {
