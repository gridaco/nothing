...
export const toSkTypeFace: PropsConverter<TypeFace, SkTypeface> = (
  canvasKit,
  typeFace
) =>
  typeFace
    ? canvasKit.SkFontMgr.RefDefault().MakeTypefaceFromData(typeFace.data)
    : undefined;
export const toSkFont: PropsConverter<Font, SkFont> = (canvasKit, font) =>
  font
    ? new canvasKit.SkFont(
        font.typeFace === undefined
          ? null
          : toSkTypeFace(canvasKit, font.typeFace)!!,
        font.size
      )
    : undefined;
export const toSkColor: PropsConverter<Color | string, SkColor> = (
  canvasKit,
  color
) => {
...
export const toFontStyle: PropsConverter<CkFontStyle, FontStyle> = (
  canvasKit,
  fontStyle
): FontStyle => {
  return {
...
export const toSkTextStyle: PropsConverter<TextStyle, SkTextStyle> = (
  canvasKit,
  textStyle
) => {
  return {
    backgroundColor:
      toSkColor(canvasKit, textStyle?.backgroundColor) ?? canvasKit.WHITE,
    ...
    foregroundColor:
      toSkColor(canvasKit, textStyle?.foregroundColor) ?? canvasKit.BLACK,
  };
};

export const toSkParagraphStyle: PropsConverter<
  ParagraphStyle,
  SkParagraphStyle
> = (canvasKit, paragraphStyle) => {
  const textAlign = paragraphStyle?.textAlign
    ...
    : undefined;
  const textDirection = paragraphStyle?.textDirection
