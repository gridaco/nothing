 # Contributing to nothing | Web Platform Tests
...
```sh
cd /path/to/nothing/..  # parent of the grida/ checkout
...
That's it — the `wpt` invocation below handles pair-matching against the built binary. Build first: `cargo build -p grida_wpt --release`, then run from the sibling wpt checkout: `./wpt run --binary=<this-repo>/target/release/grida_wpt --log-wptreport=<this-repo>/target/wpt-report.json --log-mach=- --log-mach-level=info grida <target>`.
...
```sh
# target examples (substitute <target> above):
#   css/css-transforms/2d-rotate-001.html  — one file (pilot test)
#   css/css-transforms/                    — one subsuite
#   css/                                   — all of css/ (default)
```
