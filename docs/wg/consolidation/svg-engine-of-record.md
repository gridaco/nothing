...
- **The corpus** is 692 Chromium-baked primitive cells plus 10 sampled frames.
...
both noise formulas and a procedural displacement map. A procedural image is a
floating image fact through blend arithmetic, including across a color-space
conversion; treating it as an ordinary generated byte image changes 349 to 490
pixels at maximum deltas 1 to 6 in three measured blend cases. Active sRGB
morphology directly over that image establishes a byte-domain boundary, while
blend followed by morphology retains the stronger procedural-composition
provenance. The opposite operation order is an exact control. The final
procedural layer therefore uses an architecture-stable floating restore, while
sRGB displacement uses the established exact byte restore. Generated solids
and earlier byte-domain filter outputs retain their existing policies. Both
turbulence formulas in both color spaces, both operation orders, and the
noise-to-displacement chain are exact on the current ARM host without a
tolerance.
...
by morphology by 393 pixels at delta 1. Eighty-three committed Chromium cells
carry the admitted slice without a new tolerance. They cover
...
at delta 1. This is the same CPU-family final-restore class seen in earlier
filter rungs, not a formula or sampling disagreement. Scoped exact displacement
restore and the floating procedural restore keep all 692 ARM cells exact. The
hosted-x86 confirmation is pending; this paragraph records the failed run and
the candidate separately rather than treating local architecture agreement as
proof.

Three focused rows join the refusal register, moving it from 140 to 143. The
primitive corpus moves from 609 to 692 cells; the ten exact-time sampled frames
