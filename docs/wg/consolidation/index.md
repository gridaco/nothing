...
| [the text oracle method](./text-oracle.md) | finding | how text is gated on the engine of record: the deterministic-font rung, the admitted numeric domain, the hermetic font environment, the corpus-growth law | **ratified** 2026-08-04 ([gridaco/nothing#68](https://github.com/gridaco/nothing/pull/68)); binds the text rungs; the D-M text stage it left open closed low 2026-08-05; real-font geometry Rung B, T3a direct clusters/default pair positioning, T3b precomposed Latin mappings, T3c bounded combining clusters/glyph offsets, T4a paint-only source runs, T4b positioned chunks, T5a ordered declared-family selection, T5b exact static face descriptors, T5c static nearest-face selection, and T6 cluster-safe declared-family fallback exercised by 2026-09-04 |
...
of the n0 path is [the Web checklist](./web-checklist.md) — the
HTML, CSS, and SVG surface as one box-per-construct tracker in the
platform's own vocabulary, ticked by each rung's docs commit. The other three docs are
