...
| 2 | **Shaped text** — SVG text/tspan, then HTML inline text, through each engine's own text producer under the ratified [text-oracle method](./text-oracle.md) (the D-M text stage joined **low**: no neutral shaped-text artifact enters the shared contract; fonts stay a host-seam content-digest declaration) | SVG vector exit; the two-producer text spike required by D-M exists; the relevant text/font oracle decision is registered before choosing a bar | Structure/geometry/computed-style assertions and the legally available text pixel gate pass on the engine-of-record route (`websem → rframe → n0`, outline-realized, gated byte-exact under the text-oracle method); n0's private text stays gated by its own duty-cycle oracles — the low join keeps its glyph replay policy engine-private, and its measured anti-alias fringe is that policy, not a divergence from an external gate it never claimed |
...
| AMD | Paint-RFD amendments (diamond-gradient extension; tri-state run-fill) | Phase 1; both remain open | named owner + drafted amendment text; each gates any leaf whose conformance depends on the amendment and deletion of the corresponding mapping, not the unrelated solid-fill proving scope |
| D-C | For paint and stroke leaves carried by the D-M-admitted glyphless vector facts, `cg` is the shared leaf-vocabulary seat; adoption remains per-leaf rather than retaining duplicate leaves behind permanent mappings. This is not a wholesale type swap: packed AARRGGBB color, grouped repeatable strokes, and direct RFD-law gates remain mandatory. AMD-dependent and text-paint leaves remain open | **taken for the glyphless vector scope** 2026-07-23 | the [paint-vocabulary conformance gap report](./paint-vocabulary-gap.md); later AMD-dependent and text leaves retain their own evidence bars |
...
| D-M | Staged n0 join per fact kind. **Vector stage:** source-neutral glyphless visual facts join high into the shared downstream: one engine-private compiler, private drawlist, painter, damage policy, and cache policy; SVG meaning stays Web-owned and the proving-shell downstream does not survive as a peer engine. **Text stage: shaped text joins low** — each engine keeps its own text artifact, font registry, glyph replay policy, and text item; sharing stops at backend glyph/raster utilities, and content-digest font identity stays a host-seam convention, never a contract fact | **vector stage taken** 2026-07-23, coupled to D-C for the admitted facts; **text stage taken** 2026-08-05 | Vector: the [n0-join-point finding](./n0-join-point.md), [Web renderer adoption patrol](./web-renderer-adoption.md), D-C paint/stroke gap report, complete n0 compiler-read inventory, and independently constructed normalized-input spike proving drawlist/raster equality plus multi-frame identity, damage, cache, and mixed-fact composition. Text: the two-producer spike `crates/n0/src/text_join_spike.rs` and [the text-stage evidence](./n0-join-point.md#the-text-stage-evidence) — glyph identities join bit-exactly, placement and metric facts arrive scaler-quantized per platform from n0's backend, glyph replay is pixel-visible raster policy, and the fact is a resource reference the contract refuses |
...
| D-H | Text-oracle identity: stay on `skparagraph@skia-0.99.0` vs fonts-backed production oracle | Phase 5 boundary only | crates/fonts contract + a differential run |
