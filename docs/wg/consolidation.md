...
description: "The post-landing program that consolidates the legacy engine (crates/grida) and the v2 engine (crates/n0) into one engine — phases, gates, decision registry."
tags:
  - internal
  - wg
  - plan
...
Tracking: [gridaco/nothing#9](https://github.com/gridaco/nothing/issues/9)
(the v2 model program) and the umbrella issue filed at Phase 0.
Predecessor: the legacy seam program
([gridaco/nothing#27](https://github.com/gridaco/nothing/issues/27), complete).
...
|---|---|---|---|
| GO | Merge #5 | now | CI green on the tip; sweep evidence current |
...
| FLIP | Promotion/flip rule (per-suite thresholds, coverage requirements, oracle-discipline clause) | Phase 2 — before any score exists | scoreboard v0 design + corpus enumeration |
| NAME | SVG IR crate name + its math vocabulary (math2 vs kurbo) | Phase 3 entry | naming exercise per doctrine |
| D-D | htmlcss font-provider seam (the one open M4 lock) | Phase 4 HTML entry (study starts Phase 0) | the WG study |
| D6 | Editor-core ownership: grida_editor vs n0 journal/ops ([#1](https://github.com/gridaco/nothing/issues/1)) | Phase 4, concurrent lane | timeboxed OP-catalog mapping spike against the graduated spec (scoped subset, not the full catalog) |
...
| D-E/D-I | Per-domain bar flips + the wasm switch (package identity, soak, grida-side unpin, freeze-contract retirement) | Phase 6 | the Phase-2 criterion read off the board |
...
grounded in the seam program's M4 extraction-readiness review, the
