//! Producer contract for ordered typed paints in Draft 0 `.n0.xml`.
...
fn typed_fill_lowers_to_the_ordered_production_variants() {
    let doc = n0_xml::parse(THREE_LAYER).expect("typed fills parse");
...
fn canonical_writer_preserves_rich_fill_order_and_uses_the_family_gradient() {
    let doc = n0_xml::parse(THREE_LAYER).unwrap();
    let printed = n0_xml::print(&doc).expect("rich paints print");
...
fn shorthand_and_explicit_empty_fill_have_one_canonical_form() {
    ...
    let default_printed = n0_xml::print(&defaulted).unwrap();
    assert!(!default_printed.contains(" fill="), "{default_printed}");
    assert!(!default_printed.contains("<fill"), "{default_printed}");

    let empty = n0_xml::parse(
...
fn gradient_renderability_boundary_is_enforced_during_source_ingestion() {
    ...
    let error = n0_xml::parse(&singular).unwrap_err();
    assert!(
    ...
    for distance in ["0.0000152587890625", "0.000030517578125"] {
        ...
        let error = n0_xml::parse(&degenerate).unwrap_err();
        assert!(
...
fn linear_gradient_uv_boundary_is_exact_or_explicitly_unrepresentable() {
    ...
    let printed = n0_xml::print(&doc).expect("finite f64-representable alignments print exactly");
    assert_eq!(doc, n0_xml::parse(&printed).unwrap());
...
fn writer_refuses_unspellable_gradient_model_state_without_repair() {
    ...
    let mut too_few = n0_xml::parse(source).unwrap();
...
fn frozen_textir_is_singleton_solid_and_rich_state_fails_writers() {
    ...
    let rich = n0_xml::parse(THREE_LAYER).unwrap();
    ...
    assert!(matches!(
        n0_xml::print(&unsupported_image),
