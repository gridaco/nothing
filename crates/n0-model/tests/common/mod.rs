...
pub(crate) const EPS: f32 = 1e-3; // N-3 within-platform tolerance for the lab
...
pub(crate) fn opts_visual() -> ResolveOptions {
    ResolveOptions {
        viewport: (1000.0, 1000.0),
...
pub(crate) fn run(doc: &Document) -> Resolved {
    resolve(doc, &opts())
}

pub(crate) fn assert_close(a: f32, b: f32, msg: &str) {
    assert!(
        (a - b).abs() < EPS,
        "{msg}: {a} vs {b} (Δ={})",
        (a - b).abs()
...
pub(crate) fn assert_rect(r: RectF, x: f32, y: f32, w: f32, h: f32, msg: &str) {
...
pub(crate) fn shape(w: f32, h: f32) -> (Header, Payload) {
    (
...
pub(crate) fn frame_free(w: SizeIntent, h: SizeIntent) -> (Header, Payload) {
    (
        Header::new(w, h),
        Payload::Frame {
