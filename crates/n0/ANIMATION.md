...
| `n0::svg_animation_frame`     | Thin composition of the moving latest-profile compiler, separate explicit Base and Sample entries, resolver-intent refusal, and identity-view checked paint                                                                  | General SVG import, implicit Base/Sample fallback, allocation, clear color, I/O, cadence, report publication, or oracle policy | Reusable adapter for diagnostic hosts that deliberately follow the latest proving profile                                              |
...
frame seam through different pacing policies. Exact-time diagnostic hosts use
the producer-owned `svg_animation_frame` composition rather than copying that
policy or inheriting either proving host wholesale.
...
- `svg_animation_frame` compiles the moving latest proving profile once, keeps
  authored Base distinct from explicit-time Sample, and rejects ignored or
  erroneous resolver intent before identity-view paint while leaving storage
  and publication to its caller;
...
- `tests/svg_animation_frame.rs` covers latest-profile compilation, static
  authored Base, explicit-time Sample, their intentional distinction,
  caller-owned storage, ordinary frame-product return, and strict
  unsupported-source refusal; module tests pin structured resolver-intent
  refusal; and
