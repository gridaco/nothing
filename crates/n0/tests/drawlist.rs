...
fn tag(k: &ItemKind) -> &'static str {
    match k {
        ...
        ItemKind::BeginMaskContent => "mask-content-begin",
        ItemKind::BeginMaskSource { .. } => "mask-source-begin",
        ItemKind::EndMaskSource => "mask-source-end",
...
fn frame_and_line_have_no_implicit_ink() {
    ...
    let resolved = resolve(&doc, &opts());
    assert!(build_glyphless_unchecked(&doc, &resolved).items.is_empty());
}
...
fn unsupported_rectangular_stroke_states_emit_no_draw_item() {
    for desc in [ShapeDesc::Ellipse, ShapeDesc::Rect] {
        ...
        let resolved = resolve(&doc, &opts());
        assert!(
            build_glyphless_unchecked(&doc, &resolved).items.is_empty(),
