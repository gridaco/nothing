...
cargo run -p grida_dev -- photo.png

# open with a remote URL
cargo run -p grida_dev -- https://example.com/scene.grida
```
...
cargo run -p grida_dev --release -- bench ./fixtures/test-grida/bench.grida --width 5000 --height 5000

# control frame count
cargo run -p grida_dev --release -- bench --frames 500
```
...
Examples live under `crates/grida_dev/examples/*` (with the CLI now covering the former `app_*` flows).
