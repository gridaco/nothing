...
fn hidden_elements_do_not_false_alarm_on_unconsumed_properties() {
    let hidden = admit_both(&document(
        r##"  <rect x="8" y="8" width="24" height="24" fill="#16a34a" visibility="hidden" style="stroke-opacity: 0.5"/>"##,
    ));
    assert_eq!(
        hidden.nodes.len(),
        0,
    ...
    SvgFrameSource::from_standalone_svg(
        document(
            r##"  <rect x="8" y="8" width="24" height="24" fill="#16a34a" style="stroke-opacity: 0.5"/>"##,
        ),
        viewport(),
    )
