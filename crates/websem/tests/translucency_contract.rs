...
//! rounding byte-exactly. Element `opacity` graduated with the group-scope
//! rung: where it folds, it joins this same one product — the laws live in
//! `opacity_contract.rs`.
...
// The `element_opacity_stays_a_named_refusal` law graduated with the
// group-scope rung: the construct is consumed, and its laws — the fold
// joining this file's one product, the scope where folding would
// double-blend — live in `opacity_contract.rs`.
