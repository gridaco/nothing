//! Mature static HTML/CSS/SVG → Skia Picture renderer.
//!
...
/// Explicitly Skia-backed font environment consumed by the mature HTML path.
///
/// The renderer only reads the caller's configured [`FontCollection`]. Font
/// loading, fallback policy, resource identity, and invalidation remain host
/// concerns. This transitional seam deliberately does not claim to be a
/// source-neutral font contract.
pub trait SkiaFontCollectionProvider {
    fn font_collection(&self) -> &skia_safe::textlayout::FontCollection;
}

impl SkiaFontCollectionProvider for skia_safe::textlayout::FontCollection {
    fn font_collection(&self) -> &skia_safe::textlayout::FontCollection {
        self
    }
}
...
pub fn render(
    ...
    width: f32,
    _height: f32,
    fonts: &dyn SkiaFontCollectionProvider,
    ...
    let fonts = fonts.font_collection();
...
pub fn render_svg(svg: &str, width: f32, height: f32) -> Result<skia_safe::Picture, String> {
    crate::svg::render_to_picture(svg, width, height)
...
pub fn measure_content_height(
    ...
    Ok(layout::compute_content_height(
        &root,
        width,
...
mod tests {
    ...
    fn test_fonts() -> skia_safe::textlayout::FontCollection {
        ...
        fonts.set_asset_font_manager(Some(embedded_provider.clone().into()));
        fonts.set_dynamic_font_manager(Some(user_provider.into()));
        fonts.set_test_font_manager(Some(fallback_provider.into()));
        fonts.set_default_font_manager(Some(embedded_provider.into()), None);
        fonts
    }
...
"#;
    ...
    fn test_text_decoration_style_variants() {
        let _guard = crate::stylo_test::lock();
        use cg::prelude::TextDecorationStyle as TDS;
        let cases: &[(&str, TDS)] = &[
            ("solid", TDS::Solid),
            ("double", TDS::Double),
            ("dotted", TDS::Dotted),
