...
pub fn build_shader(ctx: &PaintCtx<'_>, node: NodeId, bbox: Rect) -> Option<Shader> {
    ...
    let (mut attrs, content_node) = collect_inherited(ctx.dom, ctx.resources, node, 0)?;
    ...
    let (svg_vp_w, svg_vp_h) =
        crate::svg::layout::viewport::nearest_svg_viewport(ctx, pattern_node);
    ...
    if let Some(t) = attrs.pattern_transform {
        let pattern_node = ctx.dom.node(node);
        let origin = crate::svg::layout::transform::transform_origin_in_box(
            pattern_node,
            (0.0, 0.0, tile.width(), tile.height()),
        );
        attrs.pattern_transform = Some(crate::svg::layout::transform::wrap_with_origin(&t, origin));
    }
