...
fn flatten_glyphs(
    ...
    let viewport = crate::svg::layout::viewport::nearest_svg_viewport(ctx, root);
...
fn read_text_length(ctx: &PaintCtx<'_>, node: &DemoNode, font_size: f32) -> Option<f32> {
    ...
    if let Some(num) = lower.strip_suffix('%') {
        ...
        let (vw, vh) = crate::svg::layout::viewport::nearest_svg_viewport(ctx, node);
...
fn build_text_fill_paint(
    ...
    if let Some(v) = value {
        if v.starts_with("url(") {
            ...
            let bbox = run_bbox(run, font);
            let viewport = crate::svg::layout::viewport::nearest_svg_viewport(ctx, node);
            ...
            if let Some(trail) = crate::svg::resources::paint_server::paint_fallback(v) {
                if let Some(Paint::Color(c)) = crate::svg::dom::attrs::parse_paint(trail) {
                    paint.set_color(c);
                    if opacity_factor < 1.0 {
        ...
        let color = match crate::svg::dom::attrs::parse_paint(v) {
...
fn resolve_text_paint_order(ctx: &PaintCtx<'_>, node: &DemoNode) -> [TextPhase; 2] {
    use crate::svg::style::cascade::cascade_property;
...
fn read_inherited(ctx: &PaintCtx<'_>, node: &DemoNode, name: &str) -> Option<String> {
    ...
    for use_id in crate::svg::paint::scoped_svg_paint_state::use_chain_iter(ctx.use_chain) {
