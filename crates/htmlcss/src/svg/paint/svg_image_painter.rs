...
//! [`crate::ImageProvider`] for external URLs), then emits
...
fn rasterize_inline_svg(
    ...
    let no_fonts = crate::svg::PreloadedFonts::new();
    let inner_ctx = crate::svg::RenderContext::new(ctx.images, &crate::svg::NoCss, &no_fonts);
    let pic =
        crate::svg::render_to_picture_with_context(xml, svg_w as f32, svg_h as f32, inner_ctx)
            .ok()?;
