...
//!
//! Plugs into this crate for both standalone `.svg` documents and inline
//! `<svg>` in HTML. Static-only. V1 is a direct `DemoDom` →
...
//!
//! This module is a renderer, not an import adapter or source converter.
//!
//! # Status (checkpoint 1)
//!
...
///
/// Used by [`crate::paint::paint_inline_svg`] for inline
...
/// pass `&crate::NoImages` when none are available.
pub fn render_into(
    canvas: &skia_safe::Canvas,
    svg_xml: &[u8],
    viewport: skia_safe::Rect,
    images: &dyn crate::ImageProvider,
) -> Result<(), SvgError> {
