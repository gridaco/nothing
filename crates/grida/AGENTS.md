...
# headless GPU example (grida only, requires native-gl-context feature)
cargo run -p grida --example headless_gpu --features native-gl-context --release
```

## Package Docs

```sh
cargo doc --package grida --open --no-deps
```
