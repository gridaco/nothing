...
/// Draw a rectangle filled with a sweep gradient at the specified position and size.
///
/// # Arguments
...
/// * `gradient` - The sweep gradient paint to fill the rectangle with
fn draw_rect(
    ...
    let paint = sweep_gradient_paint(gradient, size);
...
fn main() {
    ...
    // Create a base sweep gradient
    let sweep_gradient = SweepGradientPaint {
        stops: vec![
            GradientStop {
                offset: 0.0,
                color: CGColor::from_rgba(255, 97, 97, 255).into(),
            },
            GradientStop {
                offset: 0.5,
                color: CGColor::from_rgba(133, 0, 0, 255).into(),
            },
            GradientStop {
                offset: 1.0,
    ...
    for i in 0..8 {
        ...
        draw_rect(canvas, current_x, y, (rect_width, height), &sweep_gradient);
    ...
    let sweep_gradient_translated = SweepGradientPaint {
    ...
    let sweep_gradient_rotated = SweepGradientPaint {
        stops: sweep_gradient.stops.clone(),
        opacity: sweep_gradient.opacity,
    ...
    dev_kit::save_golden(&mut surface, "gradient_sweep_transform");
}
